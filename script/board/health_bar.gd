...
@onready var def_scale: Vector2
@onready var hpvfs: Vector2
...
func update_hp(new_hp: int) -> void:
	...
	get_tree().create_tween().tween_property(
		hp_value, "scale:x", hpvfs.x * get_hp_ratio(), 0.6).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_QUAD)
